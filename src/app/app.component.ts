...
export class AppComponent {
  ...
  constructor() {
    ...
    // SERIES_DATA__NEGADAS.findIndex(([_, num]) => num === Math.max(...SERIES_DATA__NEGADAS.map(([timestamp, value]) => value)));
    ...
    this.chartOptions = {
      ...
      dataLabels: {
        enabled: true,
        // aqui habilitei apresentar os dataLabels apenas na segunda SERIES(negadas)
        enabledOnSeries: [ 1 ],
        // podia ser assim:
        // formatter: (val: string|number|number[], { seriesIndex, dataPointIndex, w }): string|number => ...
        // limpei e usei como abaixo! explicando, as SERIES sao apenas numeros, aqui e' tratado apenas eixo X
        // o val recebe o valor do item
        formatter: (val: number, { seriesIndex, dataPointIndex, w }): string => {
          // console.log('val'); console.log(val)
          // console.log('seriesIndex'); console.log(seriesIndex)
          // console.log('dataPointIndex'); console.log(dataPointIndex)
        ...
        style: {
          ...
          fontWeight: '100',
          colors: [(options: any) => {
            // console.log('dataLabels.style.colors#1')
            // console.log(options)
            // return options.series[options.seriesIndex][options.dataPointIndex] < NEGADAS_MAX ? 'red' : 'green'
      ...
      tooltip: {
        ...
        //   return `<div class="ap-tooltip-custom"><span>${series[seriesIndex][dataPointIndex]}</span></div>`;
        // },
        custom: [function({ series, seriesIndex, dataPointIndex, w }) {
        ...
        // theme: false,
        // tirei o style com css, nao funcionava ... direito mesmo
        // style: {
        //   fontSize: '12px',
        //   fontFamily: CHART_GLOBAL_FONT_FAMILY
        // },
    ...
    this.chartOptionsBrush = {
      ...
      // tentei modificar apenas o YAXIS do grafico brush... mas nao rolou, aplica nos dois, nao sei pq!
      // yaxis: DO_NOT_TOUCH__CHART_YAXIS_OPPOSITE.map((axis: ApexYAxis) => {
      //   if (axis.tickAmount) {
      //     axis.tickAmount = axis.tickAmount / 10;
      //   }
      //   return axis;
      // })
      //
